...
interface PlatformIconsProps {
  value: any; // Can be string, string[], or object with value/customText
  customText?: string;
}

const PlatformIcons: React.FC<PlatformIconsProps> = ({ value, customText }) => {
  const { t } = useLanguage();
  ...
  if (Array.isArray(value)) {
    platforms = value.map((p: string) => p.toLowerCase());
  } 
  // If value is an object with value and customText properties
  else if (typeof value === 'object' && value !== null) {
    // Use the object's customText if available, otherwise fallback to the passed customText
    const text = value.customText || customText || '';
    platforms = text.split(',').map((p: string) => p.trim().toLowerCase());
  } 
  // If value is 'custom' with customText, use the customText
  else if (value === 'custom' && customText) {
    platforms = customText.split(',').map((p: string) => p.trim().toLowerCase());
  } 
  // Otherwise just use the value as a string
  else {
    platforms = value.toString().split(',').map((p: string) => p.trim().toLowerCase());
  }
    ...
    <div className="flex flex-wrap gap-1 justify-center">
      {platforms.map((platform, index) => {
        if (platform === 'ios' || platform === 'macos' || platform === 'mac') {
          ...
          // Get translated platform name
          const platformName = t('platforms.whatsapp', {}, 'WhatsApp');
          return (
            <FeatureTooltip key={index} title={platformName} value={platform}>
              <span className="inline-flex items-center justify-center h-6 w-6 rounded-full">
                {/* WhatsApp icon in WhatsApp green */}
